...
# CORS setup: Apne Blogger ka URL yahan zaroor badlein
CORS(app, origins=[
    "https://ythubyt.blogspot.com", 
    "http://127.0.0.1:5500",
    "http://localhost:5000"
])
...
@app.route('/')
def index():
    return jsonify({"status": "Vercel API Server is live", "ffmpeg": "Not required for links"})
        ...
        for fmt in formats_raw:
            if fmt.get('ext') == 'mp4' and fmt.get('height') and fmt.get('url'):
                ...
                ui_formats.append({
                    ...
                    'size': size_str,
                    'type': 'video',
                    'url': fmt['url'] # Direct Google Server Link
                })
